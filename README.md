...
#### 🔄 自动重连功能（v2.2.0新增）
1. **连接控制面板**：在图像传输标签页中新增连接控制面板
2. **实时状态监控**：
   - 连接状态实时显示（未连接、连接中、已连接等）
   - 状态颜色指示：灰色（未连接）、橙色（连接中）、绿色（已连接）
   - 每秒自动更新连接状态
3. **自动重连设置**：
   - 默认启用自动重连功能
   - 最大重连次数：5次（可配置）
   ...
   - 用户可通过复选框启用/禁用自动重连
4. **手动重连控制**：
   - "立即重连"按钮：手动触发重连，重置重连计数
   - 智能按钮状态：根据连接状态自动启用/禁用
5. **重连流程**：
   - 检测到连接断开 → 启动重连定时器 → 尝试重新连接
   - 连接成功 → 重置重连计数 → 继续正常工作
   - 达到最大重连次数 → 停止自动重连 → 提示用户手动操作
